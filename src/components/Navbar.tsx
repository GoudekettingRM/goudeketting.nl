...
export default function Navbar() {
  ...
  return (
    <div className='fixed top-5 right-5 flex sm:flex-row'>
      <motion.ul
        initial={{ opacity: 0 }}
        animate={isOpen ? 'open' : 'closed'}
        variants={menuVariants}
        className='bg-white absolute right-0 top-[calc(100%+0.5rem)] sm:relative sm:bg-transparent flex sm:flex-row flex-col py-2 rounded-md shadow-md sm:py-0 sm:shadow-none sm:rounded-none text-left'
      >
        {links.map((link) => {
          return (
            <Link
